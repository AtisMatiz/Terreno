...
**Sources**: `mercadolivre`, `pgfn`, `olx`, `chavesnamao`/`imovelweb`/`wimoveis` (all via `htmlportal.py`), `caixa` (public CSV + `curl_cffi` for its bot wall), `brave` (long-tail via Brave Search API), `facebook`. `vivareal` is **disabled** (`fontes.vivareal: false`, 2026-08-11) — the portal misbehaves in an ordinary browser too, so it was not worth fixing. Profiles: `ci` = `brave, chavesnamao, caixa`; `local` = everything else plus those, run from the user's own machine via `scripts/run_local.sh`.

**Measured reality of each source, from a real residential run (2026-08-11)** — this is the table that replaces the old blanket "datacenter IP" story:

| Source | From the user's Mac | Cause |
|---|---|---|
| `caixa` | ✅ 200 | works |
| `chavesnamao` | ✅ 43 listings | works |
| `wimoveis` | ✅ 46 listings (one URL 404s, then it recovers) | works; the diagnostic's own probe URL returns 530, which is the portal's backend erroring, not a block on us |
| `olx`, `imovelweb` | ❌ 403 | genuine block; fingerprinting not yet ruled in or out |
| `pgfn` | ❌ `SSLZeroReturnError` | refused at the TLS handshake |
| `mercadolivre` | ❌ 403 | **not a block** — API needs OAuth, `ML_ACCESS_TOKEN` unset |
...
1. **⏳ BLOCKED ON THE USER — the one thing worth doing first: `python3 -m pip install curl_cffi && python3 scripts/diagnostico.py` on the home Mac, then paste the output.** Three real `http.py` bugs were found and fixed (see Known bugs), but whether they actually clear OLX/imovelweb/PGFN is **unknowable from this sandbox**: it routes through a TLS-terminating proxy that replaces the very fingerprint under test, so any result measured here is noise. Their first attempt failed because `pip3` installed into a different Python than `python3` runs — hence the `-m`. Everything downstream (whether to keep chasing fingerprinting, or accept a self-hosted runner / paid proxy / Brave-only coverage) depends on that table. **Do not guess further without it.**
2. **The user wants a button, not a terminal** — stated explicitly and not yet delivered. Two candidate paths: (a) finish the existing Vercel + `api/disparar.js` wiring so the published page's "rodar busca agora" works end-to-end (but that triggers the *CI* profile, which by definition excludes every locally-blocked source, so it does not actually replace the local run); (b) a double-clickable macOS app (Automator/`.command` wrapper around `scripts/run_local.sh`) for the local half. **(b) is the one that matches what they asked for**; (a) is a separate, already-half-built feature. A one-off Terminal run has now succeeded, so the GitHub credential is cached in Keychain and a button would not re-prompt.
3. **Widen search criteria** in `criteria.yaml` — still narrow test values (SP/Vale do Paraíba, 2–10 ha, R$100k–2M). With most portals blocked, this is the highest-leverage lever left on result count, and it costs nothing.
4. **Facebook/Apify payload fixed but never exercised against a live call** — schema now matches the actor's declared input. Unverified without a real `APIFY_TOKEN`: whether those Marketplace URL forms return anything. City slugs are derived from municipality names, right for most Brazilian municipalities but not guaranteed, and a wrong slug yields an empty page rather than an error. The run log prints the URLs used; `APIFY_FB_START_URLS` overrides them by hand.
5. **Verify `sites_descobertos` end-to-end on a real Brave run** — promotion at the 2-occurrence threshold and the weekly `ultima_consulta` gate are unit-tested against `Store` directly, but no real host has been observed promoting through `brave_visit.py` → `brave_discover.py` yet.
6. **Watch `brave_pendentes` failure counts over the next few runs** — retry-then-discard (`falhas`, threshold 2) was seen queueing 171 retries but a full discard cycle has never been observed live.
7. **PGFN residential-IP tunnel** — only if item 1 shows PGFN is genuinely IP-blocked rather than fingerprint-blocked. Cloudflare Tunnel / Tailscale Funnel from a home machine, routing only `comprei.pgfn.gov.br` via `requests`' `proxies=`.
8. **Mercado Livre needs a free OAuth token** (`ML_ACCESS_TOKEN`, app at developers.mercadolivre.com.br) — its 403 is auth, not blocking, so no network work will ever fix it.
