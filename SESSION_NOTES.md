...
7. **OLX, Mercado Livre and VivaReal still returned HTTP 403, and PGFN still timed out, on the user's first real `--profile local` run from their home Mac** (2026-08-11) — contradicts the whole "it's just datacenter IPs" assumption these profiles were built on. Not yet investigated: could be the specific ISP/IP, missing headers/cookies the scrapers don't send, or these sites having tightened detection generally. Needs a look before trusting `local` unconditionally.
