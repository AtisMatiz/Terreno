...
8. **Facebook/Apify call failing with `"Field input.startUrls is required"` (HTTP 400)** — surfaced in the same first local run; the Apify actor call in `terreno/sources/facebook.py` is missing a required input field. Not fixed yet.
