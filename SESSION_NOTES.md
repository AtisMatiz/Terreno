...
2. **Check the ZenRows HTTP 422 (`RESP001`) on olx/imovelweb/mercadolivre-api** — the diagnostic's response snippet was truncated before the actual reason. Could be a free-plan limitation (premium_proxy gated off Free — flagged as unverified in the original research) or a genuine target refusal. Worth knowing before assuming ZenRows can never help those three.
